------------
-- RUSSIAN BY StingerSoft --
------------


if (GetLocale() == "ruRU") then

-------------------
-- Compatibility --
-------------------

HEALBOT_DRUID   = "Друид";
HEALBOT_HUNTER  = "Охотник" or "Охотница";
HEALBOT_MAGE    = "Маг";
HEALBOT_PALADIN = "Паладин";
HEALBOT_PRIEST  = "Жрец" or "Жрица";
HEALBOT_ROGUE   = "Разбойник" or "Разбойница";
HEALBOT_SHAMAN  = "Шаман" or "Шаманка";
HEALBOT_WARLOCK = "Чернокнижник" or "Чернокнижница";
HEALBOT_WARRIOR = "Воин";

HEALBOT_HEAVY_NETHERWEAVE_BANDAGE = "Плотные бинты из ткани Пустоты";
HEALBOT_HEAVY_RUNECLOTH_BANDAGE   = "Плотные бинты из рунной ткани";
HEALBOT_MAJOR_HEALING_POTION      = "Хорошее лечебное зелье";
HEALBOT_PURIFICATION_POTION       = "Зелье Очищения";
HEALBOT_ANTI_VENOM                = "Противоядие";
HEALBOT_POWERFUL_ANTI_VENOM       = "Мощное противоядие";
HEALBOT_ELIXIR_OF_POISON_RES      = "Elixir of Poison Resistance";
HEALBOT_STONEFORM                 = "Каменная форма";

HEALBOT_FLASH_HEAL          = "Быстрое исцеление";
HEALBOT_FLASH_OF_LIGHT      = "Вспышка Света";
HEALBOT_GREATER_HEAL        = "Большее исцеление";
HEALBOT_BINDING_HEAL        = "Совместное исцеление"
HEALBOT_PRAYER_OF_MENDING   = "Молитва восстановления";
HEALBOT_HEALING_TOUCH       = "Целительное прикосновение";
HEALBOT_HEAL                = "Исцеление";
HEALBOT_HEALING_WAVE        = "Волна исцеления";
HEALBOT_HOLY_LIGHT          = "Свет небес";
HEALBOT_LESSER_HEAL         = "Малое исцеление";
HEALBOT_LESSER_HEALING_WAVE = "Малая волна исцеления";
HEALBOT_POWER_WORD_SHIELD   = "Слово силы: Щит";
HEALBOT_REGROWTH            = "Восстановление";
HEALBOT_RENEW               = "Обновление";
HEALBOT_REJUVENATION        = "Усиленное омоложение";
HEALBOT_LIFEBLOOM           = "Жизнецвет";
HEALBOT_PRAYER_OF_HEALING   = "Молитва исцеления";
HEALBOT_CHAIN_HEAL          = "Цепное исцеление";

HEALBOT_POWER_WORD_FORTITUDE    = "Слово силы: Стойкость";
HEALBOT_PRAYER_OF_FORTITUDE     = "Молитва Стойкости";
HEALBOT_DIVINE_SPIRIT           = "Божественный дух";
HEALBOT_PRAYER_OF_SPIRIT        = "Молитва духа";
HEALBOT_SHADOW_PROTECTION       = "Защита от темной магии";
HEALBOT_PRAYER_OF_SHADOW_PROTECTION = "Молитва Защиты от темной магии";
HEALBOT_INNER_FIRE            = "Внутренний огонь";
HEALBOT_INNER_FOCUS           = "Внутреннее сосредоточение";
HEALBOT_FEAR_WARD             = "Защита от страха";
HEALBOT_TOUCH_OF_WEAKNESS     = "Прикосновение слабости";
HEALBOT_ARCANE_INTELLECT      = "Чародейский интеллект";
HEALBOT_ARCANE_BRILLIANCE     = "Чародейская гениальность";
HEALBOT_FROST_ARMOR           = "Морозный доспех";
HEALBOT_ICE_ARMOR             = "Ледяной доспех";
HEALBOT_MAGE_ARMOR            = "Магический доспех";
HEALBOT_MOLTEN_ARMOR          = "Раскаленный доспех";
HEALBOT_DEMON_ARMOR           = "Демонический доспех";
HEALBOT_DEMON_SKIN            = "Шкура демона";
HEALBOT_FEL_ARMOR             = "Доспех Скверны";
HEALBOT_DAMPEN_MAGIC          = "Ослабление магии";
HEALBOT_AMPLIFY_MAGIC         = "Усиление магии";
HEALBOT_SHADOW_GUARD          = "Страж Тьмы";
HEALBOT_DETECT_INV            = "Обнаружение невидимости";

HEALBOT_LIGHTNING_SHIELD      = "Щит молний";
HEALBOT_ROCKBITER_WEAPON      = "Оружие Камнедробителя";
HEALBOT_FLAMETONGUE_WEAPON    = "Оружие языка пламени";
HEALBOT_FROSTBRAND_WEAPON     = "Оружие ледяного клейма";
HEALBOT_EARTH_SHIELD          = "Щит Земли";
HEALBOT_WATER_SHIELD          = "Водный щит";
HEALBOT_WIND_FURY             = "Оружие неистовства ветра";

HEALBOT_MARK_OF_THE_WILD      = "Знак дикой природы";
HEALBOT_GIFT_OF_THE_WILD      = "Дар леса";
HEALBOT_THORNS                = "Шипы";
HEALBOT_OMEN_OF_CLARITY       = "Знамение ясности";

HEALBOT_BLESSING_OF_MIGHT       = "Благословение могущества";
HEALBOT_BLESSING_OF_WISDOM      = "Благословение мудрости";
HEALBOT_BLESSING_OF_SALVATION   = "Благословение спасения";
HEALBOT_BLESSING_OF_SANCTUARY   = "Благословение неприкосновенности";
HEALBOT_BLESSING_OF_LIGHT       = "Благословение Света";
HEALBOT_BLESSING_OF_PROTECTION  = "Благословение защиты";
HEALBOT_BLESSING_OF_FREEDOM     = "Благословение cвободы";
HEALBOT_BLESSING_OF_SACRIFICE   = "Благословение жертвенности";
HEALBOT_BLESSING_OF_KINGS       = "Благословение королей";
HEALBOT_GREATER_BLESSING_OF_MIGHT     = "Великое благословение могущества";
HEALBOT_GREATER_BLESSING_OF_WISDOM    = "Великое благословение мудрости";
HEALBOT_GREATER_BLESSING_OF_KINGS     = "Великое благословение королей";
HEALBOT_GREATER_BLESSING_OF_LIGHT     = "Великое благословение Света";
HEALBOT_GREATER_BLESSING_OF_SALVATION = "Великое благословение спасения";
HEALBOT_GREATER_BLESSING_OF_SANCTUARY = "Великое благословение неприкосновенности";
HEALBOT_RIGHTEOUS_FURY          = "Праведное неистовство";
HEALBOT_DEVOTION_AURA           = "Аура благочестия";
HEALBOT_RETRIBUTION_AURA        = "Аура возмездия";
HEALBOT_CONCENTRATION_AURA      = "Аура сосредоточенности";
HEALBOT_SHR_AURA                = "Аура сопротивления темной магии";
HEALBOT_FRR_AURA                = "Аура сопротивления магии льда";
HEALBOT_FIR_AURA                = "Аура сопротивления огню";
HEALBOT_CRUSADER_AURA           = "Аура воина Света";
HEALBOT_SANCTITY_AURA           = "Аура святости";

HEALBOT_INTERVENE               = GetSpellInfo(3411);

HEALBOT_A_MONKEY            = "Дух обезьяны";
HEALBOT_A_HAWK              = "Дух ястреба";
HEALBOT_A_CHEETAH           = "Дух гепарда";
HEALBOT_A_BEAST             = "Дух зверя";
HEALBOT_A_PACK              = "Дух стаи";
HEALBOT_A_WILD              = "Дух дикой природы";
HEALBOT_A_VIPER             = "Дух гадюки";
HEALBOT_MENDPET             = "Лечение питомца";

HEALBOT_UNENDING_BREATH     = "Нескончаемое дыхание";
HEALBOT_HEALTH_FUNNEL       = "Канал здоровья";

HEALBOT_RESURRECTION       = "Воскрешение";
HEALBOT_REDEMPTION         = "Дух воздаяния";
HEALBOT_REBIRTH            = "Возрождение";
HEALBOT_ANCESTRALSPIRIT    = "Дух предков";

HEALBOT_PURIFY             = "Очистить";
HEALBOT_CLEANSE            = "Очищение";
HEALBOT_CURE_POISON        = "Выведение яда";
HEALBOT_REMOVE_CURSE       = "Снятие проклятья";
HEALBOT_ABOLISH_POISON     = "Выведение яда";
HEALBOT_CURE_DISEASE       = "Излечение болезни";
HEALBOT_ABOLISH_DISEASE    = "Снятие болезни";
HEALBOT_DISPEL_MAGIC       = "Рассеивание заклинаний";
HEALBOT_REMOVE_LESSER_CURSE = "Снятие малого проклятия";
HEALBOT_DISEASE            = "Болезнь";
HEALBOT_MAGIC              = "Магичкеский";
HEALBOT_CURSE              = "Проклятье";
HEALBOT_POISON             = "Яды";
-- HEALBOT_DISEASE_en         = "Disease";  -- Do NOT localize this value.
-- HEALBOT_MAGIC_en           = "Magic";  -- Do NOT localize this value.
-- HEALBOT_CURSE_en           = "Curse";  -- Do NOT localize this value.
-- HEALBOT_POISON_en          = "Poison";  -- Do NOT localize this value.
-- HEALBOT_CUSTOM_en          = "Custom";  -- Do NOT localize this value. 

HEALBOT_DEBUFF_ANCIENT_HYSTERIA = "Древняя истерия";
HEALBOT_DEBUFF_IGNITE_MANA      = "Воспламенение маны";
HEALBOT_DEBUFF_TAINTED_MIND     = "Запятнанный разум";
HEALBOT_DEBUFF_VIPER_STING      = "Жало гадюки";
HEALBOT_DEBUFF_SILENCE          = "Молчание";
HEALBOT_DEBUFF_MAGMA_SHACKLES   = "Оковы магмы";
HEALBOT_DEBUFF_FROSTBOLT        = "Ледяная стрела";
HEALBOT_DEBUFF_PSYCHIC_HORROR   = "Глубинный ужас";
HEALBOT_DEBUFF_HUNTERS_MARK     = "Метка охотника";
HEALBOT_DEBUFF_SLOW             = "Замедление";
HEALBOT_DEBUFF_ARCANE_BLAST     = "Чародейская вспышка";
HEALBOT_DEBUFF_IMPOTENCE        = "Проклятие бессилия";
HEALBOT_DEBUFF_DECAYED_STR      = "Ослабление силы";
HEALBOT_DEBUFF_DECAYED_INT      = "Угасание интеллекта";
HEALBOT_DEBUFF_CRIPPLE          = "Увечье";
HEALBOT_DEBUFF_CHILLED          = "Окоченение";
HEALBOT_DEBUFF_CONEOFCOLD       = "Конус холода";
HEALBOT_DEBUFF_CONCUSSIVESHOT   = "Шокирующий выстрел";
HEALBOT_DEBUFF_THUNDERCLAP      = "Удар грома";
HEALBOT_DEBUFF_HOWLINGSCREECH   = "Визгливый вой";
HEALBOT_DEBUFF_DAZED            = "Замедление";
HEALBOT_DEBUFF_FALTER           = "Шаткая поступь";
HEALBOT_DEBUFF_UNSTABLE_AFFL    = "Нестабильное чернокнижие";
HEALBOT_DEBUFF_DREAMLESS_SLEEP  = "Мирный сон";
HEALBOT_DEBUFF_GREATER_DREAMLESS = "Больший мирный сон";
HEALBOT_DEBUFF_MAJOR_DREAMLESS  = "Старший мирный сон";
HEALBOT_DEBUFF_FROST_SHOCK      = "Ледяной шок";

HEALBOT_RANK_1              = "(Уровень 1)";
HEALBOT_RANK_2              = "(Уровень 2)";
HEALBOT_RANK_3              = "(Уровень 3)";
HEALBOT_RANK_4              = "(Уровень 4)";
HEALBOT_RANK_5              = "(Уровень 5)";
HEALBOT_RANK_6              = "(Уровень 6)";
HEALBOT_RANK_7              = "(Уровень 7)";
HEALBOT_RANK_8              = "(Уровень 8)";
HEALBOT_RANK_9              = "(Уровень 9)";
HEALBOT_RANK_10             = "(Уровень 10)";
HEALBOT_RANK_11             = "(Уровень 11)";
HEALBOT_RANK_12             = "(Уровень 12)";
HEALBOT_RANK_13             = "(Уровень 13)";

HB_SPELL_PATTERN_LESSER_HEAL     = "Исцеление цели на (%d+)-(%d+) ед.";
HB_SPELL_PATTERN_HEAL            = "Исцеление цели на (%d+)-(%d+) ед.";
HB_SPELL_PATTERN_GREATER_HEAL    = "Медленно накладываемое заклинание, которое излечивает одну цель на (%d+)-(%d+) ед.";
HB_SPELL_PATTERN_FLASH_HEAL      = "Исцеление дружественной цели на (%d+) - (%d+) ед.";
HB_SPELL_PATTERN_RENEW           = "Излечение цели на (%d+) - (%d+) ед. в течение (%d+) сек";
HB_SPELL_PATTERN_RENEW1          = "Излечение цели на (%d+) ед. в течение (%d+) сек";
-- HB_SPELL_PATTERN_RENEW2              = "Not needed for en";
-- HB_SPELL_PATTERN_RENEW3              = "Not needed for en";
HB_SPELL_PATTERN_SHIELD          = "поглощает (%d+) урона. В течении (%d+) сек.";
HB_SPELL_PATTERN_HEALING_TOUCH   = "Исцеление дружественной цели на (%d+)-(%d+) ед.";
HB_SPELL_PATTERN_REGROWTH        = "Исцеление дружественной цели на (%d+)-(%d+) ед. и еще (%d+) ед. в течение (%d+) сек";
HB_SPELL_PATTERN_REGROWTH1       = "Исцеление дружественной цели на (%d+)-(%d+) ед. и еще (%d+) - (%d+) ед. в течение (%d+) сек";
HB_SPELL_PATTERN_HOLY_LIGHT      = "Исцеляет дружественную цель на (%d+)-(%d+)ед. здоровья";
HB_SPELL_PATTERN_FLASH_OF_LIGHT  = "Исцеление дружественной цели на (%d+)-(%d+) ед.";
HB_SPELL_PATTERN_HEALING_WAVE    = "Исцеление дружественной цели на (%d+)-(%d+) ед.";
HB_SPELL_PATTERN_LESSER_HEALING_WAVE = "Исцеление дружественной цели на (%d+)-(%d+) ед.";
HB_SPELL_PATTERN_REJUVENATION    = "Излечение цели на (%d+) ед. в течение (%d+) сек";
HB_SPELL_PATTERN_REJUVENATION1   = "Излечение цели на (%d+)-(%d+) ед. в течение (%d+) сек";
-- HB_SPELL_PATTERN_REJUVENATION2   = "Излечение цели на (%d+)-(%d+) ед. в течение (%d+) сек";
HB_SPELL_PATTERN_MEND_PET        = "Восполнение здоровья питомца на (%d+)  ед. в течение (%d+) сек";

HB_TOOLTIP_MANA          = "^(%d+) Маны$";
HB_TOOLTIP_RANGE          = "(%d+) ярдов";
HB_TOOLTIP_INSTANT_CAST      = "Мгновенное действие";
HB_TOOLTIP_CAST_TIME        = "(%d+.?%d*) сек применения";
HB_TOOLTIP_CHANNELED        = "Channeled";
HB_OFFLINE                   = "В не сети"; -- has gone offline msg
HB_ONLINE                   = "В сети"; -- has come online msg
HB_HASLEFTRAID           = "^([^%s]+) покинул группу рейда$";
HB_HASLEFTPARTY          = "^([^%s]+) покинул группу$";
HB_YOULEAVETHEGROUP        = "Вы покинули группу.";
HB_YOULEAVETHERAID         = "Вы покинули группу рейда";
HB_YOUJOINTHERAID         = "Вы вступили в рейд";
HB_YOUJOINTHEGROUP         = "Вы вступили в группу";

-----------------
-- Translation --
-----------------

HEALBOT_ADDON = "HealBot " .. HEALBOT_VERSION;
HEALBOT_LOADED = " загружен.";

HEALBOT_CASTINGSPELLONYOU = "Чтение заклинания %s на вас ...";
HEALBOT_CASTINGSPELLONUNIT = "Чтение заклинания %s на %s'а...";

HEALBOT_ACTION_OPTIONS  = "Настройки";

HEALBOT_OPTIONS_TITLE          = HEALBOT_ADDON;
HEALBOT_OPTIONS_DEFAULTS       = "Стандарт";
HEALBOT_OPTIONS_CLOSE          = "Закрыть";

HEALBOT_OPTIONS_PANEL_TEXT     = "Настройки панели Исцеления"
HEALBOT_OPTIONS_BARALPHA       = "Прозрач-ть включеных";
HEALBOT_OPTIONS_BARALPHAINHEAL = "Прозр-ть вход-го исцеления";
HEALBOT_OPTIONS_BARALPHAEOR    = "Прозр-ть 'Вне досягаемости'";
HEALBOT_OPTIONS_ACTIONLOCKED   = "Закрепить позиции";
HEALBOT_OPTIONS_AUTOSHOW       = "Авто закрыть";
HEALBOT_OPTIONS_PANELSOUNDS    = "Звук при открытии";
HEALBOT_OPTIONS_HIDEOPTIONS    = "Скрыть кнопку настроек";
HEALBOT_OPTIONS_PROTECTPVP     = "Избегать случайного PvP флажка";
HEALBOT_OPTIONS_HEAL_CHATOPT   = "Настройки чата";

HEALBOT_OPTIONS_SKINTEXT       = "Шкурка";
HEALBOT_SKINS_STD              = "Стандарт";
HEALBOT_OPTIONS_SKINTEXTURE    = "Текстура";
HEALBOT_OPTIONS_SKINHEIGHT     = "Высота";
HEALBOT_OPTIONS_SKINWIDTH      = "Ширина";
HEALBOT_OPTIONS_SKINNUMCOLS    = "Колонок";
HEALBOT_OPTIONS_SKINNUMHCOLS   = "Заголовки/колонки";
HEALBOT_OPTIONS_SKINBRSPACE    = "Промежуток строк";
HEALBOT_OPTIONS_SKINBCSPACE    = "Промежуток рядов";
HEALBOT_OPTIONS_EXTRASORT      = "Сорт панелей";
HEALBOT_SORTBY_NAME            = "Имя";
HEALBOT_SORTBY_CLASS           = "Класс";
HEALBOT_SORTBY_GROUP           = "Группа";
HEALBOT_SORTBY_MAXHEALTH       = "Макс Здоровья";
HEALBOT_OPTIONS_NEWDEBUFFTEXT  = "Нов.Дебафф"
HEALBOT_OPTIONS_DELSKIN        = "Удалить";
HEALBOT_OPTIONS_NEWSKINTEXT    = "Нов.Шкурка";
HEALBOT_OPTIONS_SAVESKIN       = "Сохранить";
HEALBOT_OPTIONS_SKINBARS       = "Опции панели";
HEALBOT_OPTIONS_SKINPANEL      = "Цвета панели";
HEALBOT_SKIN_ENTEXT            = "Включить";
HEALBOT_SKIN_DISTEXT           = "Выключить";
HEALBOT_SKIN_DEBTEXT           = "Отриц.Эффект";
HEALBOT_SKIN_BACKTEXT          = "Фон";
HEALBOT_SKIN_BORDERTEXT        = "Края";
HEALBOT_OPTIONS_SKINFHEIGHT    = "Размер шрифта";
HEALBOT_OPTIONS_BARALPHADIS    = "Прозрач-ть отключеных";
HEALBOT_OPTIONS_SHOWHEADERS    = "Показ заголовки";

HEALBOT_OPTIONS_ITEMS          = "Предметы";
HEALBOT_OPTIONS_SPELLS         = "Заклинания";

HEALBOT_OPTIONS_COMBOCLASS     = "Клавиши для";
HEALBOT_OPTIONS_CLICK          = "Клик";
HEALBOT_OPTIONS_SHIFT          = "Shift";
HEALBOT_OPTIONS_CTRL           = "Ctrl";
HEALBOT_OPTIONS_ENABLEHEALTHY  = "Всегда включено";

HEALBOT_OPTIONS_CASTNOTIFY1    = "Нет сообщать";
HEALBOT_OPTIONS_CASTNOTIFY2    = "Только себе";
HEALBOT_OPTIONS_CASTNOTIFY3    = "Извещать цель";
HEALBOT_OPTIONS_CASTNOTIFY4    = "Группа";
HEALBOT_OPTIONS_CASTNOTIFY5    = "Рейд";
HEALBOT_OPTIONS_CASTNOTIFY6    = "В канал";
HEALBOT_OPTIONS_CASTNOTIFYRESONLY = "Извещать только воскрешение";
HEALBOT_OPTIONS_TARGETWHISPER  = "Шепнуть цели при исцелении";

HEALBOT_OPTIONS_CDCBARS        = "Цвета панелей здоровья";
HEALBOT_OPTIONS_CDCCLASS       = "Монитор классов";
HEALBOT_OPTIONS_CDCWARNINGS    = "Стандарт сообщения";
HEALBOT_OPTIONS_SHOWDEBUFFWARNING   = "сообщения о отриц.эффектах";
HEALBOT_OPTIONS_SOUNDDEBUFFWARNING  = "Звук при отриц.эффектах";
HEALBOT_OPTIONS_SOUND1         = "Звук 1";
HEALBOT_OPTIONS_SOUND2         = "Звук 2";
HEALBOT_OPTIONS_SOUND3         = "Звук 3";

HEALBOT_OPTIONS_HEAL_BUTTONS   = "Панели исцелений:";
HEALBOT_OPTIONS_SELFHEALS      = "Себя"
HEALBOT_OPTIONS_PETHEALS       = "Питомцев"
HEALBOT_OPTIONS_GROUPHEALS     = "Группу";
HEALBOT_OPTIONS_TANKHEALS      = "Главных танков";
HEALBOT_OPTIONS_TARGETHEALS    = "Цели";
HEALBOT_OPTIONS_EMERGENCYHEALS = "Рейд";
HEALBOT_OPTIONS_ALERTLEVEL     = "Уровень тревоги";
HEALBOT_OPTIONS_EMERGFILTER    = "Экстра полосы для";
HEALBOT_OPTIONS_EMERGFCLASS    = "Настройка классов для";
HEALBOT_OPTIONS_COMBOBUTTON    = "Кнопка";
HEALBOT_OPTIONS_BUTTONLEFT     = "Левая";
HEALBOT_OPTIONS_BUTTONMIDDLE   = "Средняя";
HEALBOT_OPTIONS_BUTTONRIGHT    = "Правая";
HEALBOT_OPTIONS_BUTTON4        = "Кнопка4";
HEALBOT_OPTIONS_BUTTON5        = "Кнопка5";

HEALBOT_CLASSES_ALL            = "Все классы";
HEALBOT_CLASSES_MELEE          = "Ближние";
HEALBOT_CLASSES_RANGES         = "Дальние";
HEALBOT_CLASSES_HEALERS        = "Целители";
HEALBOT_CLASSES_CUSTOM         = "Клиентские";

HEALBOT_OPTIONS_SHOWTOOLTIP     = "Показ тултипов";
HEALBOT_OPTIONS_SHOWDETTOOLTIP  = "Показ инфо о заклинании";
HEALBOT_OPTIONS_SHOWUNITTOOLTIP = "Показ инфо о цели";
HEALBOT_OPTIONS_SHOWRECTOOLTIP  = "Показ Исцеление За Время";
HEALBOT_OPTIONS_SHOWPDCTOOLTIP  = "Показ назнач. клавиш";
HEALBOT_TOOLTIP_POSDEFAULT      = "По умолчанию";
HEALBOT_TOOLTIP_POSLEFT         = "Слева Healbotа";
HEALBOT_TOOLTIP_POSRIGHT        = "Справа Healbotа";
HEALBOT_TOOLTIP_POSABOVE        = "Вверху Healbotа";
HEALBOT_TOOLTIP_POSBELOW        = "Ввнизу Healbotа";
HEALBOT_TOOLTIP_POSCURSOR       = "Под Курсором";
HEALBOT_TOOLTIP_RECOMMENDTEXT   = "Исцеление За Время";
HEALBOT_TOOLTIP_NONE            = "нет доступных";
HEALBOT_TOOLTIP_ITEMBONUS       = "Бонус предметов";
HEALBOT_TOOLTIP_ACTUALBONUS     = "Актуальный бонус";
HEALBOT_TOOLTIP_SHIELD          = "Защита";
HEALBOT_TOOLTIP_LOCATION        = "Локация";
HEALBOT_WORDS_OVER              = "в течение";
HEALBOT_WORDS_SEC               = "сек";
HEALBOT_WORDS_TO                = "-";
HEALBOT_WORDS_CAST              = "Чтение";
HEALBOT_WORDS_FOR               = "для";
HEALBOT_WORDS_UNKNOWN           = "неизвестно";
HEALBOT_WORDS_YES               = "Да";
HEALBOT_WORDS_NO                = "Нет";

HEALBOT_WORDS_NONE              = "Нету";
HEALBOT_OPTIONS_ALT             = "Alt";
HEALBOT_DISABLED_TARGET         = "Цель"
HEALBOT_OPTIONS_SHOWCLASSONBAR  = "Класса на панели";
HEALBOT_OPTIONS_SHOWHEALTHONBAR = "Здоровья на панели";
HEALBOT_OPTIONS_BARHEALTHINCHEALS = "Входящее исцеление";
HEALBOT_OPTIONS_BARHEALTH1      = "в цифрах";
HEALBOT_OPTIONS_BARHEALTH2      = "в процентах";
HEALBOT_OPTIONS_TIPTEXT         = "Инфо тултип";
HEALBOT_OPTIONS_BARINFOTEXT     = "Инфо панель";
HEALBOT_OPTIONS_POSTOOLTIP      = "Тултип";
HEALBOT_OPTIONS_SHOWCLASSNAME   = "Имена";
HEALBOT_OPTIONS_BARCLASSCOLOUR  = "Панели по цвету класса";
HEALBOT_OPTIONS_BARTEXTCLASSCOLOUR1 = "Текст по цвету класса";
HEALBOT_OPTIONS_BARTEXTCLASSCOLOUR2 = "Ручная коррекция в секции \"Skin\"";
HEALBOT_OPTIONS_EMERGFILTERGROUPS   = "Группы";

HEALBOT_ONE    = "1";
HEALBOT_TWO    = "2";
HEALBOT_THREE  = "3";
HEALBOT_FOUR   = "4";
HEALBOT_FIVE   = "5";
HEALBOT_SIX    = "6";
HEALBOT_SEVEN  = "7";
HEALBOT_EIGHT  = "8";

HEALBOT_OPTIONS_SETDEFAULTS  = "Стандарт";
HEALBOT_OPTIONS_SETDEFAULTSMSG = "Сброс всех настроек на стандартные";
HEALBOT_OPTIONS_RIGHTBOPTIONS = "ПКМ открывает настройки";

HEALBOT_OPTIONS_HEADEROPTTEXT   = "Заголовки";
HEALBOT_SKIN_HEADERBARCOL       = "Цвета панелей";
HEALBOT_SKIN_HEADERTEXTCOL      = "Цвета текста";
HEALBOT_OPTIONS_BUFFSTEXT1      = "Заклинание";
HEALBOT_OPTIONS_BUFFSTEXT2      = "Проверка";
HEALBOT_OPTIONS_BUFFSTEXT3      = "Цвета панелей";
HEALBOT_OPTIONS_BUFF            = "Полож.Эффект";
HEALBOT_OPTIONS_BUFFSELF        = "на себя";
HEALBOT_OPTIONS_BUFFPARTY       = "на группу";
HEALBOT_OPTIONS_BUFFRAID        = "на рейд";
HEALBOT_OPTIONS_MONITORBUFFS    = "Пропуск полож.эффектов";
HEALBOT_OPTIONS_MONITORBUFFSC   = "также в бою";
HEALBOT_OPTIONS_ENABLESMARTCAST = "БыстроеЧтение когда в не боя";
HEALBOT_OPTIONS_SMARTCASTSPELLS = "Заклинания";
HEALBOT_OPTIONS_SMARTCASTDISPELL = "Расс-ть отр.эффект.";
HEALBOT_OPTIONS_SMARTCASTBUFF   = "Полож.эффекты";
HEALBOT_OPTIONS_SMARTCASTHEAL   = "Исцеления";
HEALBOT_OPTIONS_BAR2SIZE        = "размер панели маны";
HEALBOT_OPTIONS_SETSPELLS       = "Заклинания";
HEALBOT_OPTIONS_ENABLEDBARS     = "Включить панели";
HEALBOT_OPTIONS_DISABLEDBARS    = "Отключить панели когда в не боя";
HEALBOT_OPTIONS_MONITORDEBUFFS  = "Снятие отриц.эффектов";
HEALBOT_OPTIONS_DEBUFFTEXT1     = "Зак-ние снимающее отр.эффектов";

HEALBOT_OPTIONS_IGNOREDEBUFF          = "Игнорировать:";
HEALBOT_OPTIONS_IGNOREDEBUFFCLASS     = "Игнор. классов";
HEALBOT_OPTIONS_IGNOREDEBUFFMOVEMENT  = "Эффекты замедления";
HEALBOT_OPTIONS_IGNOREDEBUFFDURATION  = "Срок действия";
HEALBOT_OPTIONS_IGNOREDEBUFFNOHARM    = "Не вредный эффект";

HEALBOT_OPTIONS_RANGECHECKFREQ     = "Проверка досягаемости";
HEALBOT_OPTIONS_RANGECHECKUNITS    = "Макс. раненных целей за проверку досег-ти"

HEALBOT_OPTIONS_HIDEPARTYFRAMES    = "Скрыть окна группы";
HEALBOT_OPTIONS_HIDEPLAYERTARGET   = "Скрыть игрока и цель";
HEALBOT_OPTIONS_DISABLEHEALBOT     = "Отключить HealBot";

HEALBOT_OPTIONS_CHECKEDTARGET      = "Отмеченный";

HEALBOT_ASSIST   = "Ассист";
HEALBOT_FOCUS    = "Фокус";

HEALBOT_TITAN_SMARTCAST       = "БыстроеЧтение";
HEALBOT_TITAN_MONITORBUFFS    = "Монитор полож.эффектов";
HEALBOT_TITAN_MONITORDEBUFFS  = "Монитор отриц.эффектов"
HEALBOT_TITAN_SHOWBARS        = "Панели для";
HEALBOT_TITAN_EXTRABARS       = "Доп. панели";
HEALBOT_BUTTON_TOOLTIP        = "ЛКМ переключает окно настроек HealBotа";
HEALBOT_OPTIONS_SHOWMINIMAPBUTTON  = "Показ кнопки у мини-карты";
HEALBOT_OPTIONS_BARBUTTONSHOWHOT   = "Показ иконки ИзВ";
HEALBOT_OPTIONS_HOTONBAR           = "Внутри";
HEALBOT_OPTIONS_HOTOFFBAR          = "Снаружи";
HEALBOT_OPTIONS_HOTBARRIGHT        = "Справа";
HEALBOT_OPTIONS_HOTBARLEFT         = "Слева";

HEALBOT_OPTIONS_ENABLETARGETSTATUS = "Включенные настройки когда цель в бою";

HEALBOT_ZONE_AB   = "Низина Арати";
HEALBOT_ZONE_AV   = "Альтеракская долина";
HEALBOT_ZONE_ES   = "Око Бури";
HEALBOT_ZONE_WG   = "Ущелье Песни Войны";

HEALBOT_OPTION_AGGROTRACK    = "Монитор агрессии";
HEALBOT_OPTION_AGGROBAR      = "Флаш панель";
HEALBOT_OPTION_AGGROTXT      = ">> Показ текста <<";
HEALBOT_OPTION_BARUPDFREQ    = "Частота обновления";
HEALBOT_OPTION_USEFLUIDBARS  = "Исп текучие полосы";
HEALBOT_OPTION_CPUPROFILE    = "Использовать профайлер CPU (Инфо о нагрузке CPU )";
HEALBOT_OPTIONS_SETCPUPROFILERMSG = "Для того чтобы настройки вступилибы в силу необходима перезагрузка интерфейса, Готовы?";

HEALBOT_SELF_PVP             = "Своё PvP";
HEALBOT_OPTIONS_ANCHOR       = "Якорь";
HEALBOT_OPTIONS_TOPLEFT      = "Вверху слева";
HEALBOT_OPTIONS_BOTTOMLEFT   = "Внизу слева";
HEALBOT_OPTIONS_TOPRIGHT     = "Вверху справа";
HEALBOT_OPTIONS_BOTTOMRIGHT  = "Внизу справа";

HEALBOT_PANEL_BLACKLIST      = "Чёрный-Список";

HEALBOT_WORDS_REMOVEFROM     = "Удалить с";
HEALBOT_WORDS_ADDTO          = "Добавить в";
HEALBOT_WORDS_INCLUDE        = "Включая";

HEALBOT_OPTIONS_TTALPHA      = "Прозрачность";
HEALBOT_TOOLTIP_TARGETBAR    = "Панель цели";
HEALBOT_OPTIONS_MYTARGET     = "Моя цель";

HEALBOT_DISCONNECTED_TEXT    = "<DC>"

end
